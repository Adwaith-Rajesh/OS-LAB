...
int perform_sjf_np(Process ps[], EProcess e_ps[], int n) {
    ...
    for (clk = 0, curr_ps = 0, curr_eps = 0; curr_ps < n;) {
        ...
        if (idx != -1) {
            ...
            e_ps[curr_eps].st = clk;
            e_ps[curr_eps].ct = clk + ps[idx].bt;

            ps[idx].status = 1;  // (SJF-NP, executes a process completely)
            ps[idx].ct = e_ps[curr_eps].ct;
...
int main() {
    ...
    int num = perform_sjf_np(processes, e_processes, n);
